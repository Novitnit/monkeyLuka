...
export function PlaceholderScreen({
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      ...
      <main className="relative flex flex-1 flex-col items-center justify-center overflow-hidden px-4 py-16 sm:px-6 sm:py-24">
        {/* Ambient backdrop */}
        ...
        <section className="animate-rise relative w-full max-w-2xl overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.03] px-6 py-14 text-center shadow-[0_30px_90px_-24px_rgba(0,0,0,0.7)] backdrop-blur-md sm:px-14 sm:py-16">
          ...
          {icon ? (
            <div className="animate-bob relative mx-auto grid size-16 place-items-center rounded-2xl border border-white/10 bg-white/[0.05] shadow-[0_12px_30px_-12px_rgba(251,191,36,0.45)]">
              {icon}
            </div>
          ) : null}

          <p className="mt-8 text-[0.7rem] font-semibold tracking-[0.35em] text-amber-300/90 uppercase">
            {eyebrow}
          </p>
          <h1 className="mt-3 text-3xl font-bold tracking-tight text-balance text-zinc-50 sm:text-4xl">
            {title}
          </h1>
          <p className="mx-auto mt-4 max-w-md leading-relaxed text-pretty text-zinc-400">
            {description}
          </p>
